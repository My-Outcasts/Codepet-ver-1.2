...
// Token *verification* uses jose (above). Privileged Firestore *writes* — the
// /api/track ingest routes, called by a local hook with no user session — still
...
/** Lazily initialise (or reuse) the Admin SDK app used for privileged Firestore
 *  writes. A service account (FIREBASE_CLIENT_EMAIL + FIREBASE_PRIVATE_KEY) enables
 *  writes; without one it falls back to a project-id-only app. */
